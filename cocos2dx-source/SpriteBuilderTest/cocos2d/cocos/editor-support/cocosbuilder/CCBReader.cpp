...
std::string CCBReader::_resourcePath = "";
void CCBReader::setResourcePath(const char* resourcePath) {
    _resourcePath = resourcePath;
    std::vector<std::string> searchPath;
    std::string rootPath = "Published-iOS/";
    rootPath += resourcePath;
    searchPath.push_back(resourcePath);
    searchPath.push_back("Published-iOS");
    searchPath.push_back(resourcePath);
    FileUtils::getInstance()->setSearchPaths(searchPath);
    //FileUtils::getInstance()->getSearchPaths().push_back(resourcePath);
    //FileUtils::getInstance()->setSearchPaths(<#const std::vector<std::string> &searchPaths#>)
}
    ...
    if (hasPhysicsBody)
    {
        ...
        if (bodyShape == 0)
        {
                ...
                for (int j = 0; j < numPoints; j++)
                {
                    ...
                    points[j] = Point(x - offsetX, y - offsetY);
                }
        ...
        */
        ...
        body->setMass(density * shapeArea * 0.0001);
