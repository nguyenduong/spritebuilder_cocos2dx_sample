...
void LabelBMFontLoader::onHandlePropTypeFntFile(Node * pNode, Node * pParent, const char * pPropertyName, const char* pFntFile, CCBReader * ccbReader) {
    ...
    if(strcmp(pPropertyName, PROPERTY_FNTFILE) == 0) {
        ...
        if (CCBReader::getResourcePath().length() > 0) {
            std::string originFile = pFntFile;
            size_t index = originFile.find("/");
            std::string file;
            if (index != std::string::npos) {
                index++;
                file = originFile.substr(0, index);
                file += CCBReader::getResourcePath();
                file += "/";
                file += originFile.substr(index, originFile.length() - index);
            } else
                file = pFntFile;
            ((Label *)pNode)->setBMFontFilePath(file);
        } else
